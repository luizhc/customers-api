...
router.get('/businessKey/:businessKey', async (req, res) => {
    try {
        const customers = await Customer.find({ 'businessKey': { '$regex': req.params.businessKey, '$options': 'i' } });

        return res.send(customers);
    } catch (err) {
        return res.status(400).send({ error: 'Erro ao buscar cliente por chave de negócio' });
    }
})
